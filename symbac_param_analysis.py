...
import glob
import cv2
from pseudo_dev import erode, training_delta, predicting_delta
from benchmark import trench_indices

file_path = os.path.realpath(__file__)
file_path = file_path.replace('/symbac_param_analysis.py', '')
var = zarr.open("/media/ameyasu/Local Disk/SymBac/short_trench_hyperstack.zarr", mode='r')
analysis_dir = file_path + '/symbac_param_analysis_size_small'#'/home/ameyasu/cuda_ws/src/pseudo_labelling_real/symbac_param_analysis'
groundtruth_dir = file_path + '/symbac_param_analysis_size_small/groundtruth'#'/home/ameyasu/cuda_ws/src/pseudo_labelling_real/symbac_param_analysis/groundtruth'
lengths_list = [1.2, 1.5, 2.0, 2.5, 3.0, 3.5, 4.0, 4.5]
widths_list = np.linspace(7.5, 9.25, 8).tolist()
#lengths_list = [10, 12, 14, 16, 18, 20]
#widths_list = [5.5, 5.75, 6.0, 6.25, 6.5, 6.5]
widths_list = [width/10 for width in widths_list]
#lengths_list = [length/10 for length in lengths_list]
sizes_list = [(length, width) for length, width in zip(lengths_list, widths_list)]

if not os.path.exists(groundtruth_dir):
    os.makedirs(groundtruth_dir)
    os.makedirs(groundtruth_dir+"/convolutions")
    os.makedirs(groundtruth_dir+"/convolutions/masks")
    os.makedirs(groundtruth_dir+"/masks")

def save_fig(i, j_index, j, var, length, width, trench_list_length):
    image_1 = np.array(var[j,i,:,:])
    sample = Image.fromarray(image_1)
    sample.save(analysis_dir + f"/{str(length)}_{str(width)}/testing/convolutions/" + str(j_index*trench_list_length + i).zfill(5) + ".tif")
    sample.save(groundtruth_dir + "/convolutions/" + str(j_index*trench_list_length + i).zfill(5) + ".tif")

for (length, width) in sizes_list:
    if not os.path.exists(analysis_dir+f'/{str(length)}_{str(width)}'):
        os.mkdir(analysis_dir + f"/{str(length)}_{str(width)}")
    if not os.path.exists(analysis_dir+f'/{str(length)}_{str(width)}/training'):
        os.mkdir(analysis_dir + f"/{str(length)}_{str(width)}/training")
    if not os.path.exists(analysis_dir+f'/{str(length)}_{str(width)}/testing'):
        os.mkdir(analysis_dir + f"/{str(length)}_{str(width)}/testing")
        os.mkdir(analysis_dir + f"/{str(length)}_{str(width)}/testing/convolutions")
        os.mkdir(analysis_dir + f"/{str(length)}_{str(width)}/testing/masks")
    if len(os.listdir(analysis_dir+f'/{str(length)}_{str(width)}/testing/convolutions')) == 0:
        for i in range(800, 1100, 50):
            Parallel(n_jobs=-2)(delayed(save_fig)(i, j_index, j, var, length, width, 1600) for j_index, j in enumerate(trench_indices))
...
for size in sizes_list:
    ...
    print(length)
    print(width)
    if len(os.listdir(analysis_dir + f"/{str(length)}_{str(width)}/training")) == 0:
        os.makedirs(analysis_dir + f"/{str(length)}_{str(width)}/training/convolutions")
        os.makedirs(analysis_dir + f"/{str(length)}_{str(width)}/training/masks")
    if len(os.listdir(analysis_dir + f"/{str(length)}_{str(width)}/training/convolutions")) == 0:
        i = 0
        i_end = 8
        ...
        while i < i_end:
            try:
                scene_generation(length = length, length_var = 0.1, width = width, width_var = 0.1, sim_length = 600, sample_size=500, label_masks=True, initialised = True, save_dir= analysis_dir + f"/{str(length)}_{str(width)}/training/")
                i+=1
            except:
                print("glitched")
        Parallel(n_jobs=-1)(delayed(post_processing)(path) for path in glob.glob(f"{analysis_dir}/{str(length)}_{str(width)}/training/masks" + "/synth*.tif"))

for length,width in sizes_list:
    Image.fromarray(np.array(tifffile.imread(analysis_dir+"/synth_11111.tif"))).save(f"{analysis_dir}/{str(length)}_{str(width)}/training/convolutions/synth_{str(11111).zfill(5)}.tif")
    Image.fromarray(np.array(tifffile.imread(analysis_dir+"/blank_trench_mask.tif"))).save(f"{analysis_dir}/{str(length)}_{str(width)}/training/masks/synth_{str(11111).zfill(5)}.tif")
...
asha = ASHAScheduler(metric = "score", grace_period=10, mode="max", max_t=100)
error_dict = {}
for size in sizes_list:
    error_dict[f"{str(size[0])}_{str(size[1])}"] = []

search_space = {
    "sizes": tune.grid_search(sizes_list)
}


def symbac_delta_training(length, initialised):
    training_delta(epochs=50, training_set_in_use=analysis_dir + f"/{str(length)}/training/", training_set = 1, initialised = initialised, savefile=analysis_dir + f"/{length}/model/pretrained_model.hdf5")
    initialised = True
    predicting_delta(analysis_dir+f"/{length}/testing/convolutions", analysis_dir+f"/{length}/testing/masks", analysis_dir + f"/{length}/model/pretrained_model.hdf5")
    ...
    for i in range(len(prediction_maskfiles)):
        groundtruth_cells, true_cell_sizes, cell_index = get_cells(groundtruth_maskfiles, i, "groundtruth", groundtruth_maskfiles, False)
        prediction_cells, prediction_cell_sizes, prediction_cell_index = get_cells(prediction_maskfiles, i, "prediction", groundtruth_maskfiles, False)
        groundtruth_json = file_path + '/groundtruth_temp_data.json'#f'/home/ameyasu/cuda_ws/src/pseudo_labelling_real/groundtruth_temp_data.json'
        prediction_json = file_path + '/prediction_temp_data.json'#f'/home/ameyasu/cuda_ws/src/pseudo_labelling_real/prediction_temp_data.json'
...
tuner = tune.Tuner(
    ...
    run_config=air.RunConfig(local_dir="./results", name="symbac_param_analysis", failure_config=failure_config,),
